...
class Rule:
    def __init__(self, A, D):
        ...
        self.miss_attr_idx = set()  # 缺失属性值的索引
...
def calc_belief(references, numeric):
    ...
    for j in range(len(references) - 1):
        if references[j + 1] >= numeric and references[j + 1] - references[j] != 0:
...
def adjust_theta2(simi_func, rules1, rules2, n_features, sigma, band=1):
    incons_sum = 0
    incons = [0] * len(rules1)
    for i in range(len(rules1)):
        for j in range(len(rules2)):
            if i == j:
                continue
            sra = 1
            for k in range(n_features):
                if k in rules1[i].miss_attr_idx:
                    continue
                sra_temp = simi_func(rules1[i].alpha[k], rules2[j].alpha[k], band)
                sra = min(sra, sra_temp)
            src = simi_func(rules1[i].beta, rules2[j].beta, band)
            if sra <= EPS or src <= EPS:
                cons = 1
            else:
                cons = math.exp(-((sra / src - 1) ** 2) / ((1 / sra) ** 2))
    ...
    for i in range(len(rules1)):
        if incons_sum > EPS:
            tmp = [sigma[idx] for idx in rules1[i].miss_attr_idx]
            rules1[i].theta = (1 - incons[i] / incons_sum) * (1 - np.sum(tmp) / np.sum(sigma))
        else:
            rules1[i].theta = 1
...
def generate_extend_rules2(X, y, A, D, is_class=True):
    """
    result : rules.
        规则库
    """
...
def calc_active_weights(similarities, rules, n_features, sigma):
    ...
    active_weights : list(float)
       规则激活权重.
       如果零激活，则返回None
    active_weights_number : int 激活个数
    """
    active_weights = [similarities[i][0] * sigma[0] for i in range(len(rules))]
    active_weight_sum = 0.0
    for k in range(len(rules)):
        for j in range(n_features - 1):
            active_weights[k] += (similarities[k][j + 1] * sigma[j + 1])
        active_weights[k] = rules[k].theta * active_weights[k]
        active_weight_sum += active_weights[k]

    if active_weight_sum < EPS:
        return None, None
    active_weights_number = 0
    for k in range(len(rules)):
        if active_weights[k] != 0:
            active_weights_number += 1
        active_weights[k] /= active_weight_sum

    return active_weights, active_weights_number
...
def evidence_reasoning(active_weights, rules, D):
    ...
    beta = [0] * len(D)
    for j in range(len(D)):
        if fir_para_sum - len(D) * sec_para != 0:
