...
class MyProblem(ea.Problem):
    ...
    def aimFunc(self, pop):
        ...
        for i in range(Vars.shape[0]):
            A, D = process_to_pieces(self.X, self.y, self.class_num, int(Vars[i][-1]))
            brb = EDBRBClassifier(A, D, Vars[i][: -1])
...
def run_DE_algorithm(X, y, class_num, NIND=50, MAXGEN=1000):
    """
    ...
    print('### 结束参数优化 ###')
    print("")
    A, D = process_to_pieces(X, y, class_num, int(best_params[-1]))
    brb = EDBRBClassifier(A, D, best_params[:-1])
    brb = brb.fit(X, y)
    return brb
