...
class MyProblem(ea.Problem):
    ...
    def aimFunc(self, pop):
        ...
        for i in range(Vars.shape[0]):
            ...
            # 将效用中的最小值和最大值改为上界和下界
            utility_start_index = len(self.brb.A) * len(self.brb.rules) + len(self.brb.D) * len(self.brb.rules)
            for N in range(len(self.brb.A)):
                max_tmp = np.min(self.X, 0)[N]
                min_tmp = np.max(self.X, 0)[N]
                max_idx = 0
                min_idx = 0
                for k in range(len(self.brb.rules)):
                    tmp = Vars[i][- utility_start_index + N + k * len(self.brb.A)]
                    if max_tmp < tmp:
                        max_tmp = tmp
                        max_idx = - utility_start_index + N + k * len(self.brb.A)
                    if min_tmp > tmp:
                        min_tmp = tmp
                        min_idx = - utility_start_index + N + k * len(self.brb.A)
                Vars[i][max_idx] = np.max(self.X, 0)[N]
                pop.Phen[i][max_idx] = np.max(self.X, 0)[N]
                Vars[i][min_idx] = np.min(self.X, 0)[N]
                pop.Phen[i][min_idx] = np.min(self.X, 0)[N]
            ...
            res.append(np.sqrt(mean_squared_error(self.y, pre_y)))
        res = np.array(res)
        ...
        index = 0
        for i in range(0, len_D * len_rules, len_D):  # beta只和小于等于1
            col[index] = np.sum(Vars[:, np.shape(Vars)[1] - len_D * len_rules + i: np.shape(Vars)[1] - len_D * len_rules + i + len_D], axis=1) - 1
            index += 1
